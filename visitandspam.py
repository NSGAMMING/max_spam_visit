...
# Bot token
BOT_TOKEN = "7655774459:AAGH-0xiyoRTrcVuKxsdCbc1QTXIIgers_k"
...
async def visit(update: Update, context: CallbackContext):
    ...
    if chat_id not in allowed_groups:
        await update.message.reply_text("Please join our group to interact with the bot seamlessly. Here's the link to join: https://t.me/freefirelikesvip")
        return

    try:
        if len(context.args) != 2:
            await update.message.reply_text("Please provide a valid region and a numeric UID.\nUsage: /View <region> <UID> Or\nUsage: /Visit <region> <UID>")
            return
        ...
        if user_visits[user_id]['remaining'] <= 0:
            if user_id in vip_users and vip_users[user_id]['visit_limit'] > 0:
                ...
                await update.message.reply_text("❌ Daily free visit limit exceeded and no VIP allowance left.\n\n"
                                                "OWNER - @MUKESHSARKAR69 ✔️\n"
                                                "JOIN MY CHANNEL -https://t.me/mukeshsarkar01")
                return
        else:
        ...
        if response.status_code == 200:
            ...
            if data.get('message') == "Requests processed successfully":
                ...
                await update.message.reply_text(f"✅ Visit request successful for UID {uid}. Total time: {formatted_time}. Please restart the game to check.\n"
                                                "OWNER - @MUKESHSARKAR69 ✔️\n"
                                                "JOIN MY CHANNEL - https://t.me/mukeshsarkar01/n")
            else:
...
async def add_user(update: Update, context: CallbackContext):
    ...
    if user_id not in {OWNER_ID, MAIN_ADMIN_ID}:
    ...
    try:
        ...
        await update.message.reply_text(f"User {target_id} has been granted {total_limit} total visits and spams for 30 days.")
    except Exception as e:
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if data.get('message') == "Request processed successfully":
                await update.message.reply_text(f"✅ Successfully sent 200 spam requests to UID {target_id}.\n\n"
                                                "OWNER - @MUKESHSARKAR69 ✔️\n"
...
async def checkspam(update: Update, context: CallbackContext):
    ...
    if user_id in vip_users and vip_users[user_id]['days_left'] > 0:
        ...
        await update.message.reply_text(
            ...
            "OWNER - @MUKESHSARKAR69 ✔️\n"
            "JOIN MY GROUP -https://t.me/mukeshsarkar01/n"
        )
    else:
        await update.message.reply_text(
            f"🔍 Your Status:\n"
